...
sample_name = "50% EMIM NTF2 in PEGDA try 1";
your_name = "Matt";
% ------------
...
% --------- Do you want to include t0 as a parameter?
include_t0 = false;
% --------- 
...
hold on
plot(t,y)
ymodel = diffusion_moving_beam(t,sp(1),A,sp(2),nmax,sigma,dx,dy,"rlim",rlim,'t0',sp(3),'dx definition',...
...
if ~include_t0
    ...
    ub = ub(1:2);
    lb = lb(1:2);
    fh = @(D,C,t) diffusion_moving_beam(t,D,A,C,nmax,sigma,dx,dy,"rlim",rlim,...
    'dx definition',data_struct.image_processing.params.dx_definition);
    coefficients = {'D','C'};
else
    fh = @(D,C,t0,t) diffusion_moving_beam(t,D,A,C,nmax,sigma,dx,dy,"rlim",rlim,...
    't0',t0,'dx definition',data_struct.image_processing.params.dx_definition);
    coefficients = {'D','C','t0'};
end
...
cd(data_path)
tic
save(data_struct.date + "_" + data_struct.sample_name + "_analyzed_data.mat","data_struct")
toc
...
folder = 'Experiments';
page_title = '2025-05-16 Diffusion of CO2 in 50% EMIM NTF2 in PEGDA';
% ----
...
obj = labarchivesCallObj('notebook',notebook,...
    ...
    'page',page_title);
% microscope photo
caption = data_struct.date + " " + replace(data_struct.sample_name," ","_") + ...
    "_Kiralux camera photo of the sample annotated with calculated values";
figure(gel_fig.Number)
obj = obj.updateFigureAttachment('caption',caption,'figure_number',gel_fig.Number);
% uptake curve
caption = data_struct.date + " " + replace(data_struct.sample_name," ","_") + ...
    "_fitted FTIR peaks and uptake curve.";
figure(uptake_curve_fig.Number)
obj = obj.updateFigureAttachment('caption',caption,'figure_number',uptake_curve_fig.Number);
% single diffusion fitting result
caption = data_struct.date + " " + replace(data_struct.sample_name," ","_") +...
    "Single diffusion coefficient fitting: ";
...
figure(uptake_fit_result.Number)
obj = obj.updateFigureAttachment('caption',caption,'figure_number',uptake_fit_result.Number);
%% ---- END OF STEP 3 ----
